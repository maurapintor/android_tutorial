...
def process_apk(apk_file, features_out_dir, logger):
    ...
    try:
        ...
        logger.debug(e)
        logger.debug(
            f"{apk_file} processing failed in " f"{final_time - start_time}s..."
        )
        return None
    else:
        final_time = time.time()
        logger.debug(
            f"{apk_file} processed successfully in " f"{final_time - start_time}s"
        )
        return [
            f"{k}::{v}"
    ...
    try:
        ...
        list(
            map(
                ...
                (
                    permission
                    for permission in dom_permission
        ...
        list(
            map(
                lambda activity: activities.add(activity.getAttribute("android:name")),
                (
                    activity
        ...
        list(
            map(
                lambda provider: providers.add(provider.getAttribute("android:name")),
                (
                    provider
        ...
        list(
            map(
                lambda receiver: receivers.add(receiver.getAttribute("android:name")),
                (
                    receiver
        ...
        list(
            map(
                ...
                (
                    hardware_component
                    for hardware_component in dom_collection.getElementsByTagName(
                        "uses-feature"
                    )
        ...
        list(
            map(
                lambda action: intent_filters.add(action.getAttribute("android:name")),
                (
                    action
                    for action in dom_collection.getElementsByTagName("action")
            ...
            for method in (m for m in dx.get_methods() if m.get_code() is not None):
                ...
                for instruction in byte_code.get_instructions():
                    ...
                    suspicious_apis.update(
                        {e for e in android_suspicious_apis if e in instruction}
                    )
