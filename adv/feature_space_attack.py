...
class FeatureSpaceAttack:
    ...
    def _run(self, malware_features):
        ...
        unused_features = [
            feat for feat in malware_features if feat not in self.clf.input_features
        ]
        malware_features = [
            feat for feat in malware_features if feat not in unused_features
        ]
    ...
    def _build_manipulation_space(self, malware_features):
        ...
        for i in candidate_features:
            if DREBIN_FEATURES[self.clf.input_features[i - 1].split("::")[0]].add:
    ...
    def _init_attack(self, goodware_features):
        ...
        def init_population(pcls, ind_init):
            ...
            return pcls(
                ind_init(np.random.choice(c.nonzero()[1] + 1, self._n_features))
                for c in candidates
            )
    ...
    def _generate_candidates(self, goodware_features):
        ...
        vectorizer.vocabulary_ = {
            feat: idx for idx, feat in enumerate(self.clf.input_features)
        }
        ...
        if n_valid > self._n_candidates:
            ...
            mask[
                np.random.choice(
                    valid_indices, replace=False, size=n_valid - self._n_candidates
                )
            ] = False
        ...
        if candidates.shape[0] < self._n_candidates:
            ...
            indices = np.hstack(
                [
                    np.random.choice(
                        candidates.shape[1], size=self._n_features, replace=False
                    )
                    for _ in range(n)
                ]
            )
            indptr = np.arange(0, n * self._n_features + 1, self._n_features)
            rnd_rows = sp.sparse.csr_matrix(
                (data, indices, indptr), shape=(n, candidates.shape[1])
            )
